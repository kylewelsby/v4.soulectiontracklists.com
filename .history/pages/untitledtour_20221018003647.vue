  ...
  div(
    ...
    div(
      ...
      nuxt-img(
        ...
        class="rounded-xl md:rounded-2xl w-full h-[580px] inline-block object-contain"
        width="1536"
        fit="contain"
        provider="twicpics"
      )
      div(
        class="absolute z-10 top-25 left-0 bottom-0 right-0 flex flex-col items-center justify-center"
      )
        //- span(
        //-   class="mt-10 font-header text-xl md:text-4xl text-center"
        //- ) Download & Submit
        span(
          class="flex flex-col lg:flex-cp;"
        )
          SiteButton(
            class="mx-2 mt-4"
    ...
    div(
      ...
      //-     )
      //-       iframe(
      //-         src="https://www.youtube-nocookie.com/embed/-uwvSeJrKL8"
      //-         frameborder="0"
...
<script>
export default {
  name: 'SoulectionUntitled',
  async asyncData({ $content, error }) {
    const page = await $content('untitledtour')
      .fetch()
      .catch((err) => {
        error({
          statusCode: 404,
